...
export default function AddCourseForm({}){
    ...
    const createCoursesAPIURL = "https://s-r-m-s2022.herokuapp.com/api/v1/admin/create_courses";
    ...
    return(
        <View>
            <Formik 
            initialValues={{course_code:"", nameOfCourse:"",course_Descr:"",
                            len_course:"",unit_of_course:"",status_course:"",
                            departmentname:""}}
            validationSchema={reviewSchema}
            onSubmit ={(values, { resetForm }) => {
                const createCourses = async() => {

                    await axios.create({headers}).post(createCoursesAPIURL,values)
                        .then((res)=>{
                            setMessage(res?.data)
            ...
            >
                ...
                <Text style = {styles.error}>{formikProps.errors.course_code}</Text>
                 ...
                 />  
                 <Text style = {styles.error}>{formikProps.errors.unit_of_course}</Text>
                  < TextInput                       
                    style = {styles.input}
                 ...
                 />  
                 <Text style = {styles.error}>{formikProps.errors.departmentname}</Text>
